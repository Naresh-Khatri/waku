...
import { Check, ChevronDown, ChevronUp, Copy, Eye, Link } from "lucide-react";
import { useMemo, useState } from "react";
import { ColorPicker } from "./color-picker";
import { useEditor } from "./store";
import { LivePreviewPane } from "./live-preview";
...
export function ParamsCard({
  ...
  const fullUrl = buildUrl(liveUrl, draftValues, paramsSchema) ?? liveUrl;
    ...
    return (
      <>
        <div className="absolute bottom-3 left-3 z-10 inline-flex h-9 items-center gap-2 rounded-xl border border-zinc-200 bg-white px-3 text-xs text-zinc-500 shadow-md">
          <span>No params yet — click the link icon next to a field to bind one.</span>
          {liveUrl && fullUrl ? (
            <button
              onClick={() => setPreviewOpen((v) => !v)}
              className={`flex h-6 items-center gap-1 rounded-md px-2 text-[11px] ${
                previewOpen
        ...
        {previewOpen && fullUrl ? (
          <LivePreviewPane url={fullUrl} onClose={() => setPreviewOpen(false)} />
        ) : null}
      </>
    );
  }
  ...
  return (
    <>
      <div className="absolute bottom-3 left-3 z-10 w-[280px] rounded-xl border border-zinc-200 bg-white text-xs shadow-md">
        <div className="flex h-9 items-center justify-between rounded-xl px-3 text-zinc-700">
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex flex-1 items-center justify-between hover:bg-zinc-50"
          >
          ...
          {liveUrl && fullUrl ? (
            <button
              onClick={() => setPreviewOpen((v) => !v)}
              className={`ml-2 flex h-6 items-center gap-1 rounded-md px-2 text-[11px] ${
                previewOpen
                  ? "bg-indigo-50 text-indigo-700"
                  : "text-zinc-600 hover:bg-zinc-100"
              }`}
